...
def evaluate_assessment(request: AssessmentRequest) -> AssessmentResponse:
    ...
    Handles cases where all answers are zero or empty.
    """
    answers = request.answers  # e.g., {"Analytical": [2,1,2], "Creative": [1,2,1], ...}

    trait_scores = {}
    for domain, scores in answers.items():
        trait_scores[domain] = sum(scores) if scores else 0  # handle empty lists

    # If all scores are zero, return neutral response
    if all(score == 0 for score in trait_scores.values()):
